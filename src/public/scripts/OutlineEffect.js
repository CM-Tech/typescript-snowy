...
THREE.OutlineEffect = function (renderer, parameters) {
    ...
    var defaultThickness = parameters.defaultThickness !== undefined
        ...
        : 0.003;
    var defaultColor = parameters.defaultColor !== undefined
    ...
    var defaultKeepAlive = parameters.defaultKeepAlive !== undefined
        ...
        : false;
    // object.material.uuid -> outlineMaterial or object.material[ n ].uuid ->
    // outlineMaterial save at the outline material creation and release if it's
    // unused removeThresholdCount frames unless keepAlive is true.
    var cache = {};
    var removeThresholdCount = 60;
    // outlineMaterial.uuid -> object.material or outlineMaterial.uuid ->
    // object.material[ n ] save before render and release after render.
    var originalMaterials = {};
    // object.uuid -> originalOnBeforeRender save before render and release after
    // render.
    var originalOnBeforeRenders = {};
    // this.cache = cache;  // for debug copied from WebGLPrograms and removed some
    // materials
    var shaderIDs = {
    ...
    var vertexShaderChunk = [
        ...
        "	vec4 pos2 = projectionMatrix * modelViewMatrix * vec4( skinned.xyz + objectNorm" +
            "al, 1.0 );",
    ...
    var vertexShaderChunk2 = [
        ...
        "#else",
        "	gl_Position = calculateOutline( gl_Position, objectNormal, vec4( transformed, 1" +
            ".0 ) );",
        "#endif",
        "#include <fog_vertex>"
    ].join("\n");
    ...
    function createMaterial(originalMaterial) {
        ...
        if (shaderID !== undefined) {
            ...
            if (!/attribute\s+vec3\s+position\s*;/.test(originalVertexShader) || !/attribute\s+vec3\s+normal\s*;/.test(originalVertexShader)) {
                console.warn('THREE.OutlineEffect requires both vec3 position and normal attributes in vertex ' +
                    'shader, does not draw outline for ' + originalMaterial.name + '(uuid:' + originalMaterial.uuid + ') material.');
    ...
    function updateOutlineMaterial(material, originalMaterial) {
        ...
        if (outlineParameters !== undefined) {
            ...
            material.transparent = (outlineParameters.alpha !== undefined && outlineParameters.alpha < 1.0)
                ? true
